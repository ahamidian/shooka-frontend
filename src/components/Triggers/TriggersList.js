...
class TeamRowWithoutRouter extends Component {
  ...
  onEditButtonClicked = () => {
    this.props.history.push(`/triggers/${this.props.item.id}/edit`)
  };
  onDeleteButtonClicked = () => {
    getApi().delete(`triggers/${this.props.item.id}`, true)
      .then(response => {
      ...
      <div className="animated fadeIn">
        <Card style={{margin: "0px 0px 8px 0px"}}>
          <div className="card-header">
            <h3 style={{display:"contents"}}>Triggers</h3>
            <Button className="card-header-actions bg-primary" onClick={()=>this.props.history.push("/triggers/create")}>
               Add
            </Button>
          </div>
          <CardBody>
