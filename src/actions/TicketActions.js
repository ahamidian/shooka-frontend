...
export function loadTicketsByFilter( pageSize=100,page=0,filter,sorted=[],onSuccess, onFailure) {
    return dispatch => {
        let ordering="";
        if(sorted.length>0){
            sorted.map((sortOption)=>ordering=ordering+(sortOption.desc?"-":"")+sortOption.id)
...
export function fetchFilters(id, onSuccess, onFailure) {
    return dispatch => {
        return getApi().get(`triggers/`, true)
            .then(response => {
                if (onSuccess) {
                    onSuccess(response.data);
                }
